...
class VideoClipProcessorGUI:
    def __init__(self, root):
        ...
        self.multi_color_text = tk.BooleanVar(value=False)
        ...
        self.parallel_thumbs = tk.BooleanVar(value=False)

        # --- Performance Options ---
        self.use_performance_cut = tk.BooleanVar(value=True)
    ...
    def create_widgets(self):
        ...
        # Performance Options
        perf_frame = ttk.LabelFrame(main_frame, text="Performance Options", padding="5")
        perf_frame.grid(row=row, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=5)
        row += 1

        ttk.Checkbutton(perf_frame, text="Use Fast FFmpeg Cutting (Performance-Optimized)", 
                       variable=self.use_performance_cut).grid(row=0, column=0, sticky=tk.W)
        ttk.Checkbutton(perf_frame, text="Parallel Thumbnail Generation", 
                       variable=self.parallel_thumbs).grid(row=0, column=1, sticky=tk.W, padx=(20, 0))
        ...
        ttk.Checkbutton(overlay_frame, text="Multi-Color Text", 
                       variable=self.multi_color_text).grid(row=3, column=0, sticky=tk.W, pady=(5,0))
        row += 1
    ...
    def process_clips_thread(self):
        try:
            ...
            # Generate thumbnails in parallel if enabled
            if self.generate_thumbnails.get() and self.parallel_thumbs.get():
                self.message_queue.put(("log", "Generating thumbnails in parallel..."))
                self.generate_thumbnails_parallel(clips_data, Path(self.output_folder.get()))
            ...
            for idx, item in enumerate(clips_data["clips"], start=1):
                ...
                try:
                    # Use performance-optimized cut if enabled
                    if self.use_performance_cut.get():
                        temp_clip_path = tempfile.mktemp(suffix='.mp4')
                        ffmpeg_extract_subclip(str(video_path), start, end, targetname=temp_clip_path)
                        subclip = VideoFileClip(temp_clip_path)
                        item['_temp_path'] = temp_clip_path
                    else:
                    ...
                    # Generate thumbnail if not already done in parallel
                    if self.generate_thumbnails.get() and not self.parallel_thumbs.get():
                        try:
                    ...
                    # Clean up temp file
                    if '_temp_path' in item and os.path.exists(item['_temp_path']):
                        try:
                            os.remove(item['_temp_path'])
                        except:
                            pass
    ...
    # --- Enhanced Text Overlay with Multi-Color Support ---
    def apply_text_overlay(self, clip, item):
        ...
        if self.multi_color_text.get():
            # Multi-colored text implementation
            text_parts = text_str.split()
            if not text_parts:
                return clip
                
            # Create individual word images
            word_images = []
            fontsize = max(20, min(120, int(self.overlay_fontsize.get())))
            font = self.load_font(fontsize)
            ...
            for i, word in enumerate(text_parts):
                ...
                word_img = Image.new("RGBA", (500, fontsize * 2), (0, 0, 0, 0))
                draw = ImageDraw.Draw(word_img)
                draw.text((10, 0), word, font=font, fill=color, 
                         stroke_width=2, stroke_fill="black")
                
                # Crop to actual text size
                bbox = word_img.getbbox()
                if bbox:
                    word_img = word_img.crop(bbox)
                word_images.append(word_img)
            
            # Calculate total width and max height
            total_width = sum(img.width for img in word_images) + (len(word_images) - 1) * 10
            max_height = max(img.height for img in word_images) if word_images else fontsize
            
            # Combine all word images
            combined_img = Image.new("RGBA", (total_width, max_height), (0, 0, 0, 0))
            x_offset = 0
            
            for word_img in word_images:
                y_offset = (max_height - word_img.height) // 2
                combined_img.paste(word_img, (x_offset, y_offset))
                x_offset += word_img.width + 10  # Space between words
                
            txt_clip = ImageClip(np.array(combined_img)).set_duration(clip.duration)
            ...
            draw.text((10, 0), text_str, font=font, fill=color, 
                     stroke_width=2, stroke_fill="black")
            
            txt_clip = ImageClip(np.array(txt_img)).set_duration(clip.duration)
        
        # Position the overlay
        mapping = {
            "top": ("center", "top"), "center": ("center", "center"), 
    ...
    # --- Enhanced Thumbnail Generation with Parallel Support ---
    def generate_thumbnail(self, clip, item, output_dir: Path):
        ...
        try:
            if option == "start": 
                t = min(0.5, max(0.01, clip.duration * 0.01))
            elif option == "middle": 
                t = clip.duration / 2
            elif option == "end": 
                t = max(0.5, clip.duration - 0.5)
            elif option == "25%": 
                t = clip.duration * 0.25
            elif option == "75%": 
                t = clip.duration * 0.75
            elif option == "custom":
                try: 
                    t = float(self.thumb_custom_time.get())
                    t = min(max(0.0, t), clip.duration)
                except Exception: 
                    t = clip.duration / 2
            else: 
                t = clip.duration / 2
    ...
    def generate_thumbnails_parallel(self, clips_data, output_dir):
        """Generate thumbnails in parallel using ThreadPoolExecutor"""
        video_path = clips_data["src"]
        
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = []
            for item in clips_data["clips"]:
                future = executor.submit(self._generate_single_thumbnail_parallel, 
                                       video_path, item, output_dir)
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if result:
                        self.message_queue.put(("log", f"Parallel thumbnail: {result}"))
                except Exception as e:
                    self.message_queue.put(("log", f"Parallel thumbnail failed: {str(e)}"))

    def _generate_single_thumbnail_parallel(self, video_path, item, output_dir):
        """Generate thumbnail for a single clip (thread-safe)"""
        temp_path = None
        try:
            # Extract the specific clip for thumbnail
            temp_path = tempfile.mktemp(suffix='.mp4')
            ffmpeg_extract_subclip(video_path, item["start"], item["end"], targetname=temp_path)
            ...
            raise Exception(f"Thumbnail failed for {item.get('id', 'unknown')}: {str(e)}")
        finally:
            # Clean up temp file
...
class JsonPasteDialog:
    ...
    def create_widgets(self):
        ...
        self.ok_button = ttk.Button(button_frame, text="Load JSON", command=self.load_json)
  ...
  "clips": [
    ...
    def validate_json(self):
        ...
        if not content: 
            self.status_var.set("Paste JSON or use Show Sample.")
            self.ok_button.state(['disabled'])
            return False
        try: 
            parsed = json.loads(content)
        except json.JSONDecodeError as e:
            self.status_var.set(f"JSON syntax error: {str(e)}")
            self.ok_button.state(['disabled'])
            return False
        if not isinstance(parsed, dict): 
            self.status_var.set("Top-level JSON must be an object.")
            self.ok_button.state(['disabled'])
            return False
        if "src" not in parsed: 
            self.status_var.set("Missing 'src' field.")
            self.ok_button.state(['disabled'])
            return False
        if "clips" not in parsed or not isinstance(parsed["clips"], list): 
            self.status_var.set("Missing 'clips' array.")
            self.ok_button.state(['disabled'])
            return False
        for i, clip in enumerate(parsed["clips"], start=1):
            if not isinstance(clip, dict): 
                self.status_var.set(f"Clip {i} must be an object.")
                self.ok_button.state(['disabled'])
                return False
            for field in ("id","start","end","label"):
                if field not in clip: 
                    self.status_var.set(f"Clip {i} missing field '{field}'")
                    self.ok_button.state(['disabled'])
                    return False
        self.status_var.set("JSON looks valid ✓")
        self.ok_button.state(['!disabled'])
        return True
    ...
    def load_json(self):
        content = self.text_area.get(1.0, tk.END).strip()
        if not content: 
            messagebox.showerror("Error","Please paste JSON.")
            return
