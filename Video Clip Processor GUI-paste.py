...
class VideoClipProcessorGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Entry(main_frame, textvariable=self.json_file_path, width=50).grid(
        ...
        ttk.Button(main_frame, text="Paste JSON", command=self.open_json_paste_dialog).grid(row=row, column=3, padx=(5, 0))
        row += 1
        ...
        # Separator
        ttk.Separator(main_frame, orient="horizontal").grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=10)
        row += 1

        # Video format options
        format_frame = ttk.LabelFrame(main_frame, text="Video Format", padding="5")
        format_frame.grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=5)
        format_frame.columnconfigure(1, weight=1)

        ttk.Radiobutton(
            format_frame, text="Short-form (1080x1920)", variable=self.video_format, value="1080x1920"
        ).grid(row=0, column=0, sticky=tk.W)
        ttk.Radiobutton(
            format_frame, text="Landscape (1920x1080)", variable=self.video_format, value="1920x1080"
        ).grid(row=0, column=1, sticky=tk.W)
        ttk.Radiobutton(
            format_frame, text="Square (1080x1080)", variable=self.video_format, value="1080x1080"
        ).grid(row=0, column=2, sticky=tk.W)
        ttk.Radiobutton(format_frame, text="Original", variable=self.video_format, value="original").grid(
        ...
        quality_frame.grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=5)
        ...
        bitrate_combo = ttk.Combobox(
            quality_frame, textvariable=self.bitrate, values=["2000k", "4000k", "6000k", "8000k", "10000k"], width=10
        )
        ...
        audio_frame.grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=5)
        ...
        audio_bitrate_combo = ttk.Combobox(audio_frame, textvariable=self.audio_bitrate, values=["128k", "192k", "256k", "320k"], width=10)
        ...
        ttk.Label(main_frame, textvariable=self.progress_var).grid(row=row, column=0, columnspan=4, sticky=tk.W)
        row += 1

        self.progress_bar = ttk.Progressbar(main_frame, mode="determinate")
        self.progress_bar.grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=5)
        row += 1
        ...
        self.log_text.grid(row=row, column=0, columnspan=4, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(5, 0))
        main_frame.rowconfigure(row, weight=1)

    def browse_json_file(self):
        filename = filedialog.askopenfilename(
            title="Select Clips Recipe JSON File", filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
        )
        if filename:
    ...
    def open_json_paste_dialog(self):
        ...
        self.root.wait_window(dialog.dialog)  # Wait until closed

        if dialog.result:
            # Save to a temporary file so existing logic works unchanged
            temp_path = Path(self.output_folder.get()) / "_pasted_clips.json"
            try:
                with open(temp_path, "w", encoding="utf-8") as f:
                    json.dump(dialog.result, f, indent=2)
                self.json_file_path.set(str(temp_path))
                messagebox.showinfo("JSON Loaded", "Pasted JSON validated and saved successfully!")
    ...
    def write_video_file(self, clip, output_path, clip_id):
        codec = "libx264"
        audio_codec = "aac" if self.include_audio.get() else None
        audio_bitrate = self.audio_bitrate.get() if self.include_audio.get() else None
        ...
        try:
            ...
            self.message_queue.put(("log", f"FFMPEG error for {clip_id}: {str(e)}"))
            raise
...
class JsonPasteDialog:
    def __init__(self, parent):
        ...
        self.dialog.title("Paste JSON")
        self.dialog.geometry("600x450")  # taller so buttons don't get clipped
        self.dialog.transient(parent)
        self.dialog.grab_set()

        # Instructions
        ttk.Label(self.dialog, text="Paste your JSON recipe below:").pack(anchor=tk.W, padx=10, pady=5)

        # Text area for JSON
        self.text_area = scrolledtext.ScrolledText(self.dialog, wrap=tk.WORD, height=20)
    ...
    def on_ok(self):
        ...
        if not text:
            messagebox.showerror("Error", "No JSON provided.")
            return

        try:
            data = json.loads(text)
            if "src" not in data or "clips" not in data:
                raise ValueError("JSON must contain 'src' and 'clips' fields")
            self.result = data
            self.dialog.destroy()
        except Exception as e:
